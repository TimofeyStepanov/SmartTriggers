...
public class PostgresConfig {
    ...
    @Bean
    public Flux<ProcessorEntityDTO> getProcessorEntityDTOFlux(PostgresqlConnection postgresqlConnection,
    ...
    private ProcessorEntityDTO getDTOFromJSON(String processorEntityJson) {
        ObjectMapper objectMapper = new ObjectMapper();
        objectMapper.setPropertyNamingStrategy(PropertyNamingStrategies.SNAKE_CASE);
        objectMapper.configure(JsonParser.Feature.ALLOW_UNQUOTED_FIELD_NAMES, true);

        ProcessorEntityDTO processorEntityDTO = null;
        try {
            processorEntityDTO = objectMapper.readValue(processorEntityJson, ProcessorEntityDTO.class);
        } catch (JsonProcessingException e) {
            log.error(e.getMessage());
        }
        return processorEntityDTO;
    }
}
